      ...
      <nav className="hidden md:flex gap-8 items-center">
        {links.map((link) => (
          <Link key={link.path} to={link.path} className="relative group">
            <span
              className={`text-sm font-bold tracking-widest ${location.pathname === link.path ? 'text-black' : 'text-gray-500 hover:text-black transition-colors'}`}
            >
              {link.label}
            </span>
            {location.pathname === link.path && <span className="absolute -bottom-1 left-0 w-full h-0.5 bg-black" />}
          </Link>
        ))}
      </nav>
